...
<script>
	...
	function cambiarPagina(param1) {
		...
		/*if (typeof(param1)=='string') {
			$('#table_pubs_events').load('/nidopub-webapp/pubEvent/listEventsDate', {id_pub:'${params.id_pub}', currentDateEvent: param1});
		} else {
			$('#table_pubs_events').load('/nidopub-webapp/pubEvent/listEventsDate', {id_pub:'${params.id_pub}', currentDateEvent: $(param1).val()});
		}*/
		<!-- FIN DESARROLLO -->
		
		<!-- TEST Y PROD -->
		if (typeof(param1)=='string') {
			$('#table_pubs_events').load('/pubEvent/listEventsDate', {id_pub:'${params.id_pub}', currentDateEvent: param1});
		} else {
			$('#table_pubs_events').load('/pubEvent/listEventsDate', {id_pub:'${params.id_pub}', currentDateEvent: $(param1).val()});
		}
		<!-- FIN TEST Y PROD -->
...
<div class="fieldcontain ${hasErrors(bean: pubEventInstance, field: 'musicType', 'error')} required">
	...
	<g:select id="musicType" name="musicType.id" from="${com.nidoapp.nidopub.domain.PubMusicType.findAll()}" optionKey="id" required="" noSelection="['' : 'Selecciona un tipo de música']" value="${pubEventInstance?.musicType?.id}" class="one-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: pubEventInstance, field: 'dateEvent', 'error')} required">
	...
	<calendar:datePicker id="dateEventDate" dateFormat="%d/%m/%Y" showTime="false" name="dateEventDate" value="${new Date()}" onChange="cambiarPagina(this);" required="" />
	<br/>
	<g:textField name="dateEventHour" value="00" style="width:89px !important;" required=""/>
	&nbsp;:&nbsp; 
	<g:textField name="dateEventMinute" value="00" style="width:88px !important;" required=""/>
</div>
