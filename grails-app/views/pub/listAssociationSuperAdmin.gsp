			...
			<table class="table table-striped table-bordered table-advance table-hover">
				...
				<g:each in="${pubInstanceList}" status="i" var="pubInstance">
						...
						<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
							...
							<td><!--<g:link action="index" id="${pubInstance.uri}">-->${fieldValue(bean: pubInstance, field: "name")}<!--</g:link>--></td>
						
							<td><g:link controller="association" action="deletePubSuperAdmin" params="[associationId: association?.id]" id="${pubInstance.id}"><i class="icon-delete-style icon-minus-sign"></i></g:link></td>	
