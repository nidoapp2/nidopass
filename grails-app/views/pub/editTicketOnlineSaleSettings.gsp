				...
				<div class="row-fluid">
					<div class="span12">						
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="#">Editar configuración de venta de entradas online</a>
							</li>
						</ul>
											...
											<div class="span8">
												...
												<g:hasErrors bean="${cmd}">
												    <g:eachError bean="${cmd}">
															...
															<form action="${createLink(controller:'pub', action:'saveTicketOnlineSaleSettings')}" method="post">
																<input type="hidden" name="pubUri" value="${cmd ? cmd?.pubUri : pub?.uri}"/>
																<label class="control-label">Razón social</label>
																<input type="text" name="businessName" placeholder="Ej: Empresa S.L." class="m-wrap span8" value="${cmd ? cmd.businessName : pub.businessName}"/>
																<label class="control-label">Persona de contacto</label>
																<input type="text" name="contactPerson" placeholder="Ej: José Pérez" class="m-wrap span8" value="${cmd ? cmd.contactPerson : pub.contactPerson}"/>
																<label class="control-label">Teléfono de contacto</label>
																<input type="text" name="contactTelephone" placeholder="Ej: 970000000" class="m-wrap span8" value="${cmd ? cmd.contactTelephone : pub.contactTelephone}"/>
																<label class="control-label">CIF/NIF</label>
																<input type="text" name="nif" placeholder="Ej: 00000000A" class="m-wrap span8" value="${cmd ? cmd.nif : pub.nif}"/>															
																<label class="control-label">Dirección de facturación</label>
																<input type="text" name="invoiceAddress" placeholder="Ej: Av/ Navarra, 34, 50010, Zaragoza " class="m-wrap span8" value="${cmd ? cmd.invoiceAddress : pub.invoiceAddress}"/>
