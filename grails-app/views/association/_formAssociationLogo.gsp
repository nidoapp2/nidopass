<%@ page import="com.nidoapp.nidopub.domain.Association" %>



<div class="fieldcontain ${hasErrors(bean: associationInstance, field: 'photo', 'error')} ">
	<label for="photo">
		<g:message code="association.photo.label" default="Logo" />
		
	</label>
	<div id="imagenLogo" style="width:30%; text-align:right;">
	<img src="${g.createLink(controller:'resources', action: 'associationLogo', params:[association_id: user.association.id])}" alt="" />
	<br>
	<div class="fileupload fileupload-new" data-provides="fileupload">
		<div class="input-append">
			<div class="uneditable-input">
				<i class="icon-file fileupload-exists"></i> 
				<span class="fileupload-preview"></span>
			</div>
			<span class="btn btn-file">
			<span class="fileupload-new">Seleccionar foto</span>
			<span class="fileupload-exists">Cambiar</span>
			<input type="file" class="default" name="photoFile" accept="image/*">
			</span>
			<a href="#" class="btn fileupload-exists" data-dismiss="fileupload">Eliminar</a>
		</div>
	</div>
	</div>
</div>

